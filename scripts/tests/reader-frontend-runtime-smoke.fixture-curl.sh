...
case "$url" in
  */reader)
    body='lang="zh-CN" id="main-content" role="search" class="search-bar" class="skip-link" :focus-visible @media (max-width: 640px) @media (prefers-reduced-motion: reduce) /reader/manifest.webmanifest id="reader-install" beforeinstallprompt event.preventDefault() appinstalled reader-auth-pending location.replace returnTo href="/admin/operations#collection" href="/admin/operations#packages" href="/admin/operations#sources"'
    ;;
  */reader/account)
  ...
  */reader/icon-192.svg|*/reader/icon-512.svg)
    ...
    ;;
  */admin/operations)
    body='id="operations-content" id="operations-tabs" href="/admin/operations#collection" href="/admin/operations#packages" href="/admin/operations#sources" data-operations-tab="collection" data-operations-tab="packages" data-operations-tab="sources" role="tablist" id="operations-collection-form" id="operations-collection-url" /api/v1/admin/operations/overview /api/v1/admin/collection-runs operations-run-tabs data-collection-status bookTitle 采集地址： operations-run-card__summary captureDetailsOpenState restoreDetailsOpenState data-operations-details-key node("details" operations-card /api/v1/admin/books/ id="operations-package-form" /api/v1/admin/packages?limit=50 operations-package-card__progress operations-package-card EPUB 3 单文件 TXT id="operations-policy-form" id="operations-policy-book-id" operations-policy-card 当前下架书籍列表 /api/v1/admin/content/takedowns?limit=50 bookId: pendingAction.bookId /api/v1/admin/operations/alerts/history? operations-history-more operations-action-reason operations-action-suggestions 常用理由 reasonSuggestionsFor 可直接选择、修改或自行填写 data-reason-suggestion 任务已不再需要继续执行 清理已取消任务 重试 取消 cancelled-cleanup /api/v1/admin/collection-runs/cancelled/cleanup aria-live="polite" reader-auth-pending'
    ;;
  *)
    exit 1
    ;;
