...
跨模块消息事实保存在 PostgreSQL 的 `messaging.outbox_messages` 和 `messaging.inbox_messages`。业务 DbContext 通过 `ITransactionalOutboxWriter` 在同一事务中追加 Outbox；Outbox 以 `FOR UPDATE SKIP LOCKED`、lease 和失败退避支持至少一次投递，Inbox 以消息 ID、类型和载荷摘要保护重复消费与消息身份。当前 Crawler 任务创建已接入 `crawler.task.created` 最小事件，消息不包含变量或凭据引用；其他业务事件需按相同事务 seam 接入。
