using System;
using Microsoft.EntityFrameworkCore.Migrations;

#nullable disable

namespace InkFlow.Modules.Library.Infrastructure.Persistence.Migrations
{
    /// <inheritdoc />
    public partial class AddChapterMappings : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.CreateTable(
                name: "chapter_mappings",
                schema: "library",
                columns: table => new
                {
                    Id = table.Column<Guid>(type: "uuid", nullable: false),
                    SourceId = table.Column<string>(type: "character varying(128)", maxLength: 128, nullable: false),
                    ExternalChapterId = table.Column<string>(type: "character varying(512)", maxLength: 512, nullable: false),
                    SourceChapterId = table.Column<Guid>(type: "uuid", nullable: false),
                    CanonicalBookId = table.Column<Guid>(type: "uuid", nullable: false),
                    CanonicalChapterId = table.Column<Guid>(type: "uuid", nullable: false),
                    CreatedAt = table.Column<DateTimeOffset>(type: "timestamp with time zone", nullable: false)
                },
                constraints: table =>
                {
                    table.PrimaryKey("PK_chapter_mappings", x => x.Id);
                });

            migrationBuilder.CreateIndex(
                name: "IX_chapter_mappings_CanonicalChapterId",
                schema: "library",
                table: "chapter_mappings",
                column: "CanonicalChapterId");

            migrationBuilder.CreateIndex(
                name: "IX_chapter_mappings_SourceId_ExternalChapterId",
                schema: "library",
                table: "chapter_mappings",
                columns: new[] { "SourceId", "ExternalChapterId" },
                unique: true);
        }

        /// <inheritdoc />
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.DropTable(
                name: "chapter_mappings",
                schema: "library");
        }
    }
}
