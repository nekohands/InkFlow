...
- Identity：账号、Session、短期 AccessToken、RefreshToken、凭证、Role/Permission 和用户头像；公开注册的首个成功持久化账号为 Administrator，后续账号为 Reader，跨实例引导规则见 ADR 0024。
...
用户头像是 Identity 的单用户当前资料数据，落在 `identity.user_avatars` 的受限 `bytea` 列，服务端限制为 2 MiB 并按 PNG/JPEG/WebP 文件签名校验。它不进入公开内容或 CDN；只有在规模和运维证据要求下才评估迁移到 Object Storage。
...
Operations Center 提供受 `Operator` / `Administrator` 保护的告警快照读端口：它从来源能力健康、Crawler/Inbox 死信、一致性检查和 Redis 限流存储健康汇总当前告警，并以配置化阈值和有界结果输出稳定代码。未过滤的管理员快照会把不含动态 message 的稳定告警身份记录到 `operations.alert_incidents`，并把 opened/resolved 转折追加到 `operations.alert_history`；事务级 PostgreSQL advisory lock 负责跨实例去重，partial 快照不误恢复，历史按配置保留期清理。Inbox 死信只通过摘要 seam 读取，来源过滤的 Operator 快照不返回平台级 Inbox 状态。历史查询仍只对 Administrator 开放，Center UI 以时间倒序表格展示转折并用不透明游标加载更早记录；它不执行修复、不写入业务事实、不发送外部通知；生产告警路由、通知渠道和治理仍由后续运维集成负责。详见 ADR 0020。
...
- Inbox 以消息 ID 主键去重，处理成功后才写入 `ProcessedAt`；消费者崩溃或 lease 到期后允许再次领取。类型/载荷摘要不一致视为身份冲突并拒绝消费。处理失败按 `MaxAttempts` 和有界 `IMessageRetryPolicy` 写入 `AvailableAt`，达到上限写入 `DeadLetteredAt`；未到时间或已死信的消息不得再次进入 Handler。
...
- 当前 v1 由 Worker 选择 PostgreSQL 内部 relay：`PostgreSqlInboxMessagePublisher` 重建并核对类型、PayloadHash、TraceId 后幂等写入 `messaging.inbox_messages`，成功后才确认 Outbox；由于 `jsonb` 读回可能规范化文本，Outbox/Inbox 另保留受消息大小上限约束的 `RawPayload` 以稳定重建 hash。`OutboxRelayBackgroundService` 按 `Messaging:Relay` 的有界参数周期领取批次。该适配器不引入外部 MQ，重复投递由 Inbox 主键和身份核对吸收，具体 Inbox Handler/消费轮询仍需接收模块明确后接入。
- 4.62 增加 `IInboxStore.ClaimBatchAsync`、`InboxConsumerPump` 与 Worker `InboxConsumerBackgroundService`：轮询只按已注册 Handler 类型领取 Inbox，使用 `FOR UPDATE SKIP LOCKED` + lease，成功处理后确认，失败保留稳定失败码并释放 lease。Inbox 保存消息 `OccurredAt`，旧行缺失时回退 `ReceivedAt`；当前 Worker 尚未注册业务 Handler，空注册表安全 idle，不领取未知消息。配置使用 `Messaging:Inbox`，不改变既有 Crawler 任务轮询、阅读路径或 Outbox relay。
- 4.63 补齐 Inbox 失败策略：`MaxAttempts` 默认 5，失败写入有界退避 `AvailableAt`，达到上限写入 `DeadLetteredAt`；单条/批量 claim 排除未到时间和终态死信，Worker 记录 dead-lettered 计数并告警。该包只提供通用可靠性语义，不等同于具体业务 Handler 已接入。决策见 ADR 0018。
- 4.64 接入 Crawler `crawler.task.created` 业务 Handler：稳定载荷校验后回读 `CrawlerTask` 权威事实，使用按任务 ID 的 PostgreSQL 事务原子租约触发共享 `CrawlerTaskProcessor`；周期轮询与 Inbox Handler 共用任务失败/重试/死信状态机，Inbox 成功确认仍与任务提交分离并保持 at-least-once。Worker 补齐 Handler 依赖的 Canonical Book 仓储；其他 Integration Event 仍须各自注册接收者，决策见 ADR 0019。
- 4.65 将 Inbox 终态死信纳入 Operations 告警观测：`IInboxDeadLetterReader` 以有界摘要读取未处理死信，平台级告警使用独立阈值，读取失败以稳定错误和 partial 快照表示；来源过滤的 Operator 视图不查询平台级消息状态，决策见 ADR 0020。
...
CI Runtime smoke 使用 `scripts/core-slo-runtime-smoke.sh` 对四个服务面执行固定、有界的合成请求，并输出包含请求数、5xx 数、延迟样本数和 p95 的 UTC JSON 证据；每个服务面先执行一次不计入统计、但仍校验状态码的预热请求，以隔离源码 Compose 冷启动初始化，随后要求 `public_api` / `developer_api` p95 ≤ 750ms、`legado_api` / `reader` p95 ≤ 1000ms。空查询 Legado 与未授权 Developer API 用于避免真实来源和凭据依赖。该证据只能作为 Compose/CI 短窗口基线，不能替代生产 OTLP 窗口、告警、保留治理或人工验收，详见 ADR 0013。
