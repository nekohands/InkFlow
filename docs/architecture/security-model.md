...
Commercial access is layered: active users default to the versioned Free plan, an Administrator-only endpoint appends a new Entitlement Assignment for Pro/Developer, and Developer API operations require the `developer.catalog.read` entitlement. PostgreSQL is authoritative for the user-level UTC-month quota and immutable Usage Ledger; Redis quota snapshots are best-effort read acceleration only. Content responses remain read-only public Canonical data and exclude Source credentials, private records and management operations.
