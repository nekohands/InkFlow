...
Docker 发布门禁扫描 `0.159.0` 时发现其内置 `golang.org/x/crypto` 存在已修复的高危漏洞（CVE-2026-56854）。将两个 Compose 文件和 Docker 门禁统一更新到固定版本 `0.160.0`，保持其余网络暴露和运行时加固边界不变；更新后的镜像必须重新经过 Docker Trivy、Compose Runtime 和 Ubuntu VM 源码构建验证。
