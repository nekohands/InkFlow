...
7. 带 `RunId` 的后续任务入队和已领取任务启动由持久化层提供父运行事务门禁：入队先锁定并重新读取父运行，再执行去重、任务和 Outbox 写入；启动按任务→父运行顺序加锁，仅在任务启动被接受后才允许服务层推进兼容性状态。生产 EF 在同一事务内把 `Pending` 父运行与任务一起推进为 `Running`；父运行终态或缺失时取消任务且不调用执行器。无 `RunId` 的历史任务保持兼容路径。
8. Operations 的采集运行列表使用 `UpdatedAt + Id` 稳定倒序游标分页，单页上限为 100；页面继续读取后续游标并在客户端按全部生命周期状态分组，不使用无界单次查询。
9. 失败运行删除是受保护、要求理由并写入审计的不可逆命令；持久化层只接受 `Failed` 运行，在一个事务中删除该运行、采集子任务和死信明细，保留书籍、章节、正文、来源及审计事实。其他状态只能返回冲突，不得被该命令删除。
