...
日期：2026-09-03（采集/打包任务工作面增量）

## Page

- 目标用户：已认证 `Reader` / `Operator` / `Administrator`；Reader 使用采集与打包下载并查看来源状态，Operator/Administrator 额外观察死信、一致性和告警并执行受控修复。
- 页面性质：Reader 的采集/打包/来源状态使用面 + 运维角色的只读快照、告警历史和受控修复入口。Reader 调用 `GET /api/v1/admin/operations/overview` 获得不含 crawler/consistency 的来源状态快照；完整快照和 `GET /api/v1/admin/operations/alerts/history` 仍按运维权限保护，查询结果有界且按区块隔离故障。
- 非目标：自动修复、直接改数据库、把凭据/任务变量/正文载荷放入仪表盘。

## 本轮任务工作面增量（2026-09-03）

- 书籍采集使用既有 `CollectionRun` 列表和控制入口；卡片同时显示阶段、成功、进行中、待处理、失败、取消和剩余数量。
- 已停止运行提供“重试”和“取消”：重试沿用原采集地址创建新运行，取消只把停止终态转入可清理的 `Cancelled` 分类；已取消分组提供带理由确认的一键清理，删除运行、采集子任务和死信但保留书籍、正文与审计记录。
- 书籍打包新增受保护的 `GET /api/v1/admin/packages?limit=...` 有界列表，按更新时间倒序；页面刷新后仍能恢复排队、运行、完成、失败和过期任务。
- 已认证 Reader 可以创建/查看采集运行、创建/查看/下载完成书籍包；Reader 来源页签只渲染状态，不渲染来源启停、能力控制或死信操作按钮。
- 打包卡片使用可访问进度条；任务完成后显示文件名、大小、有效期和下载入口，过期任务明确不提供下载。
- 任务列表继续使用 4 秒单次定时轮询，但只有存在排队/运行任务时才继续调度；终态列表自动停止高频请求。页面仍不缓存认证响应。
- Reader 顶部导航提供独立的“采集”“下载”“来源状态”入口；Reader 仍只能查看来源快照，现有 Operator 来源级授权和 Administrator 角色继续控制来源编辑操作。
- 书籍详情页提供 EPUB、单文件 TXT 和 ZIP 选择；已有完成包使用当前认证会话直接下载，进行中任务跳转到下载页，否则创建新的打包任务。
...
- `401/403`：分别表达“会话无效/需要重新登录”和“没有对应使用、OperationsRead 或命令权限”；不把权限拒绝伪装成空数据。
...
- **Cancelled collection cleanup**：要求 `CrawlerRepair` 权限和非空理由；确认后显示影响为当前已取消运行及其采集子任务/死信，成功反馈实际删除数量，不能删除书籍、正文或审计记录。
- 所有高风险按钮默认不在列表首屏制造误触；采用明确动词、不可含糊的确认对话框、理由输入、提交中禁用和成功/失败结果。服务端权限、理由校验和审计是最终边界，UI 确认不能替代它们。

## InkFlow-specific requirements

- Reader 导航只显示采集、打包和来源状态；不显示 Source/Crawler/ContentVersion 等运维内部概念。Center 的运维页签仍按角色隐藏。
