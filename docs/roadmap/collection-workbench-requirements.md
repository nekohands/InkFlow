...
状态：需求已对齐，代码已落地；当前采集/打包工作包代码候选为 `4600d2b`，并补齐 CollectionRun 启动输入的 `400/422` 语义映射、成功进度语义、停止任务重试/取消、已取消任务批量清理、按操作提供常用理由/自定义理由，以及 Reader 的采集/打包使用权限：空/非法 URL 返回 `400`，来源地址无法解析返回 `422`，失败/取消任务不计入成功百分比。已在 Ubuntu VM 以源码构建 Compose 完成真实 PostgreSQL 测试、Migration 检查、采集/打包非阅读 App 自动化验收和操作理由弹窗浏览器验收；真实 Official Source pair、真实凭据 Operations 验收和阅读 3.0 真机链路仍按待定事项处理。
日期：2026-09-01
...
3. 对已完成且已发布的正典书生成一个可下载的书籍包；
4. 普通 Reader 可以使用采集和下载能力，但只能查看书源状态，不能执行来源运维操作。
...
| 项目 | v1 推荐方案 | 说明 |
| --- | --- | --- |
| 使用人 | 已认证 Reader / Operator / Administrator | Reader 可创建/查看采集、创建/查看/下载书籍包；来源状态只读，来源启停、能力控制和运维修复仍限 Operator / Administrator |
...
作为 Reader，我可以输入书籍地址发起采集、查看采集进度，并为符合内容政策的已完成书籍生成和下载 EPUB、单文件 TXT 或 ZIP；我可以查看来源状态，但不能停用/恢复来源或来源能力、处理死信、治理内容和执行运维清理。
...
| `Stopped` | 优雅停止完成；不再继续此运行 | 是（可转取消清理） |
| `Cancelled` | 已取消；不再重试或创建后续任务 | 是（可批量清理） |
...
| 停止 | 持久化为 `Stopping`；不再创建/领取新任务，允许当前原子章节完成 | 最终为 `Stopped`；可标记为 `Cancelled` 后清理运行记录，需要重新采集时创建新运行 |
| 取消 | 持久化为 `Cancelled`；禁止新任务、重试和后续链路。正在进行的网络请求在协作取消或安全检查点结束 | 终态，不恢复；可由运维批量清理运行及子任务，已发布历史内容不回滚 |

补充约束：

- 控制命令必须幂等；重复点击不能重复创建运行、任务或审计记录中的新业务事实。
- 控制命令本身不物理删除父运行或子任务，保留状态、时间、操作者和理由，便于审计与复盘；单独的取消清理命令才删除已取消运行及其采集子任务/死信。
- `Stopped → Cancelled` 只改变终态清理分类，不恢复运行、不创建新任务；重试通过原地址创建新的 `CollectionRun`。
...
POST /api/v1/admin/collection-runs/cancelled/cleanup
Body: { "reason": "..." }
```

控制请求统一接受 `{ "reason": "..." }`，理由长度和内容遵循现有运维命令约束。取消清理成功返回 `{ "status": "cleaned", "deletedCount": 0 }`，只删除已取消运行、其采集子任务和死信，不删除书籍、正文或审计记录。创建运行应对同一来源/外部书籍的活跃运行做幂等去重；终态运行允许明确创建新的重新采集运行。
...
- v1 不承诺图片/音频、多语言排版或按用户隔离的私有任务历史；已认证 Reader 可以使用受内容政策约束的书籍采集和下载。
...
- Reader：创建/查看采集运行、创建/查看/下载书籍包、查看受限来源状态；
- Operator / Administrator：继承 Reader 使用能力，并额外控制运行、清理任务、读取运维快照/死信、操作来源和执行内容治理；
- 每个写操作从认证主体取得 actor，不接受客户端自报操作者；
- 创建、暂停、恢复、停止、取消、取消清理、打包和下载均记录审计事件，至少包含资源 ID、前后状态、理由、结果和时间；
...
- 运行状态机覆盖合法/非法流转、幂等控制、暂停恢复、优雅停止和取消；停止运行可转为取消清理分类；
...
- API 的 202/404/409/422/401/403 状态及响应字段稳定：新建采集运行返回 202，活跃运行复用返回 200，来源地址无法解析返回 422，未知运行返回 404，非法控制状态返回 409，非法输入返回 400；取消清理 API 受 `CrawlerRepair` 保护，要求理由，返回 `cleaned` 与 `deletedCount`，并追加审计事件；
...
- v1 不交付图片/音频、多语言复杂排版或按用户隔离的私有任务历史与控制权。
...
| Q4 | 已认证 Reader / Operator / Administrator 均可创建和下载书籍包，内容政策和完成状态仍强制执行 |
| Q5 | 只接受已登记公开来源，未知或无法解析 URL 直接拒绝 |
| Q6 | 入口为受保护 API 和 `/admin/operations`；Reader 只显示采集、打包和来源状态，运维页签及来源命令不向 Reader 开放 |
...
- 将当前平台级采集/打包任务改造成按用户隔离的私有历史、配额或控制权；
