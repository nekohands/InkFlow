...
- 范围：Reader / Identity / Personal Legado Token / Profile Avatar
- 更新：2026-09-03
...
- 当前头像默认使用显示名称首字符作为占位；用户可以上传并替换自己的 JPG、PNG 或 WebP 头像，单文件不超过 2 MiB。
- 头像上传只作用于当前认证用户；服务端按文件签名检测格式，不信任文件名或客户端 MIME，并以私有、不可缓存的头像响应返回。
...
- 已登录用户可以上传有效头像并在刷新后看到结果；替换头像会覆盖当前头像，非法格式和超过 2 MiB 的文件会被拒绝。
