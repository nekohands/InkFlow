...
### 5.45 用户头像上传与私有存储交接（本轮，2026-09-03）

- 实现：账户中心个人资料页新增头像选择/上传入口，支持 JPG、PNG、WebP，成功后刷新头像，未上传时使用显示名称首字母占位图。
- API/持久化：新增认证用户自有的 `GET/PUT /api/v1/me/profile/avatar`，Identity 新增 `user_avatars` 表和 `AddUserAvatars` 迁移；按 `UserId` 原子替换当前头像，GET 设置 `private, no-store` 与 `X-Content-Type-Options: nosniff`。
- 安全边界：服务端按文件签名判定 PNG/JPEG/WebP，不信任文件名和客户端 MIME；multipart 请求体和文件均限制在 2 MiB 级别；不接受 SVG，不提供裁剪、历史、审核或对象存储。
- 本机证据：定向头像测试先红后绿；Restore、Release Build（0 warnings / 0 errors）、Unit 557/557、Architecture 1/1、Contract 10/10、11 个迁移上下文、头像/账户/前端 shell 回归、`bash -n` 与 `git diff --check` 均 PASS。WSL 包装迁移脚本因 WSL 内 dotnet 不可用未作为证据。
...
- 门禁：文档提交 `23757d0` 的 [CI 33738529085](https://github.com/nekohands/InkFlow/actions/runs/33738529085)、[Docker 33738530154](https://github.com/nekohands/InkFlow/actions/runs/33738530154)、[Security 33738530652](https://github.com/nekohands/InkFlow/actions/runs/33738530652) 均 success；不要将本地 Windows Integration 的 Docker named pipe 失败误判为代码回归。
- 人工待定：浏览器中未提交取消/清理等破坏性操作；真实上游、真实凭据、阅读 3.0/MuMu 真机和其他第 6 节人工验收继续待定。

### 5.52 Reader 采集与书籍包权限交接（本轮，2026-09-03）

- 范围：新增 `OperationsSnapshotRead`、`CollectionUse`、`BookPackageUse`；普通 Reader 可以创建/查看采集运行、创建/查看/下载 EPUB/TXT/ZIP 书籍包，并查看来源状态快照。
- 运维边界：来源启停、能力操作、死信、失败/取消清理、内容治理、告警、一致性检查及采集运行全局控制仍要求 Operator/Administrator；Reader 前端不展示这些操作按钮，只展示来源状态。
